...
// ---------------------------------------------------------------------------
// Market Research Reports — AI-generated research reports for campaign strategy
// Reports are generated by LLM based on a topic/category and stored for reuse.
// Insights from reports can be linked to specific campaigns.
// ---------------------------------------------------------------------------
export const marketResearchReports = mysqlTable("market_research_reports", {
  ...
  status: mysqlEnum("status", ["generating", "ready", "archived"]).notNull().default("generating"),
  summary: text("summary"),
  keyFindings: json("key_findings").$type<string[]>(),
  opportunities: json("opportunities").$type<string[]>(),
  risks: json("risks").$type<string[]>(),
  recommendedActions: json("recommended_actions").$type<Array<{ action: string; priority: "high" | "medium" | "low"; campaignType: string }>>(),
  sources: json("sources").$type<string[]>(),
  linkedCampaignIds: json("linked_campaign_ids").$type<string[]>(),
  generatedBy: varchar("generated_by", { length: 100 }).notNull().default("ai"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  researchCategoryIdx: index("research_category_idx").on(table.category),
  researchStatusIdx: index("research_status_idx").on(table.status),
  researchCreatedIdx: index("research_created_idx").on(table.createdAt),
}));
