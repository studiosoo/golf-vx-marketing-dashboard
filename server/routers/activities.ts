...
import { getActivityAssets, deleteActivityAsset } from "../db";
import {
  getSundayClinicData,
  getWinterClinicData,
  getJuniorCampData,
...
// Used only for the generic fallback path; known IDs use dedicated functions.
...
export const activitiesRouter = router({
  getAssets: protectedProcedure
    .input(z.object({ activityId: z.string(), venueId: z.string() }))
    .query(async ({ input }) => {
      const assets = await getActivityAssets(input.activityId, input.venueId);
      return { assets };
    }),
  ...
  deleteAsset: adminProcedure
    ...
    .mutation(async ({ input }) => {
      await deleteActivityAsset(input.id);
      return { success: true };
    }),
