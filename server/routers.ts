...
Respond in JSON:
{
  ...
  members: router({
    ...
    listWithPrograms: protectedProcedure
      .input(z.object({
        search: z.string().optional(),
        status: z.enum(["active", "inactive", "cancelled", "trial"]).optional(),
      }).optional())
      .query(async ({ input }) => {
        ...
        if (!dbConn) return [];
        const membersList = await db.getAllMembers(input);
        if (membersList.length === 0) return [];
        const memberIds = membersList.map(m => m.id);
        const { memberAppointments: maTable } = await import("../drizzle/schema");
        const aptRows = await dbConn.select().from(maTable)
          .where(and(inArray(maTable.memberId, memberIds), eq(maTable.canceled, false)))
          .orderBy(desc(maTable.appointmentDate));
        const aptsByMember = new Map<number, any[]>();
        for (const apt of aptRows) {
          if (!aptsByMember.has(apt.memberId)) aptsByMember.set(apt.memberId, []);
          aptsByMember.get(apt.memberId)!.push(apt);
        }
        return membersList.map(m => ({
          ...m,
          appointments: (aptsByMember.get(m.id) || []).slice(0, 5).map((a: any) => ({
            type: a.appointmentType as string,
            category: a.category as string,
            date: a.appointmentDate as Date,
            amountPaid: parseFloat(a.amountPaid || '0'),
          })),
          programCount: (aptsByMember.get(m.id) || []).length,
          lastProgram: (aptsByMember.get(m.id) || [])[0]?.appointmentType || null,
          lastProgramDate: (aptsByMember.get(m.id) || [])[0]?.appointmentDate || null,
        }));
      }),
