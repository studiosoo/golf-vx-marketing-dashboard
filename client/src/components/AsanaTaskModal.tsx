  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#DEDEDA]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {isDone ? (
            <div className="space-y-3">
              ...
              {createdTasks.map((t, i) => (
                <a key={i} href={t.permalink_url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 px-4 py-2.5 bg-[#FAFAFA] border border-[#DEDEDA] rounded-lg hover:bg-[#F0F0F0] transition-colors group">
            ...
            tasks.map((task, idx) => (
              <div key={task.id} className="border border-[#DEDEDA] rounded-xl p-4 space-y-3">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-[#DEDEDA] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F2DD48]/50 focus:border-[#F2DD48]"
                />
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 text-sm border border-[#DEDEDA] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F2DD48]/50 focus:border-[#F2DD48] resize-none"
                />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-sm border border-[#DEDEDA] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F2DD48]/50 focus:border-[#F2DD48] bg-white"
                    >
        ...
        <div className="px-6 py-4 border-t border-[#DEDEDA] flex items-center justify-between gap-3">
          {isDone ? (
            ...
            <>
              <button onClick={addTask} disabled={createBatch.isPending}
                className="flex items-center gap-1.5 px-3 py-2 text-sm text-[#666] border border-[#DEDEDA] rounded-xl hover:bg-[#F1F1EF] transition-colors disabled:opacity-50">
                <Plus className="w-4 h-4" />Add task
              </button>
              <div className="flex items-center gap-2">
                <button onClick={onClose} disabled={createBatch.isPending}
                  className="px-4 py-2.5 text-sm text-[#666] border border-[#DEDEDA] rounded-xl hover:bg-[#F1F1EF] transition-colors disabled:opacity-50">
                  Cancel
                </button>
