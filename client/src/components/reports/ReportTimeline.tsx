...
function TimelineRow({
  ...
  const colors = item.campaigns.map(c => CAMPAIGN_COLORS[c]?.solid ?? TEXT_M);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex-1 h-7 relative rounded" style={{ background: BG_S }}>
        ...
        {/* TBD placeholder bar (left-anchored when no dates) */}
        {!item.datesConfirmed && (
          <div
...
export function ReportTimeline({ defaultOpen = false }: ReportTimelineProps) {
  ...
  // Show items visible in this period + TBD items (always shown)
      ...
      {open && (
        <div className="px-5 pb-5" style={{ borderTop: `1px solid ${BORDER}` }}>
          ...
          <div className="flex items-center justify-between mt-4 mb-3">
            {/* Left: View mode + data source toggles */}
            <div className="flex items-center gap-2">
