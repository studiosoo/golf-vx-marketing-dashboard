...
export function BoomerangMembersTab() {
  ...
  const { data: templatesData, isLoading: templatesLoading } =
    trpc.boomerang.getTemplates.useQuery();

  // Auto-select first membership template
  useEffect(() => {
    if (!selectedTemplateId && templatesData && templatesData.length > 0) {
      const membership = (templatesData as any[]).find((t: any) => t.isMembership);
      setSelectedTemplateId((membership ?? templatesData[0] as any).id);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          ...
          <SelectContent>
            {((templatesData ?? []) as any[]).map((t: any) => (
...
export default BoomerangMembersTab;
