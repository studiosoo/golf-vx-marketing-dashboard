...
function StatusBadge({ status }: { status: ActionStatus }) {
  const map: Record<ActionStatus, { label: string; className: string }> = {
    ...
    dismissed:        { label: "Dismissed",         className: "bg-gray-50 text-[#6F6F6B] border border-gray-200" },
    execution_failed: { label: "Failed",            className: "bg-red-50 text-red-600 border border-red-200" },
    archived:         { label: "Archived",          className: "bg-gray-50 text-[#6F6F6B] border border-gray-200" },
  };
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {action.campaignName && (
              <span className="text-xs text-[#6F6F6B] bg-[#F1F1EF] px-2 py-0.5 rounded font-mono">{action.campaignName}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button onClick={() => onDismiss?.(action.id)} disabled={isLoading}
            className="p-1.5 text-[#999] hover:text-[#FF3B30] hover:bg-red-50 rounded transition-colors" title="Dismiss">
...
export default function AIActions() {
  ...
  return (
    <div className="p-6 space-y-8 bg-[#F6F6F4] min-h-full">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#111] tracking-tight">AI Actions</h1>
          <p className="text-sm text-[#6F6F6B] mt-1">Autonomous decisions and recommendations based on live campaign data{lastSync && <span className="ml-2 text-[#999]">· Last sync {lastSync}</span>}</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-white border border-[#DEDEDA] rounded-lg p-1">
            <button onClick={() => setView("active")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              view === "active" ? "bg-[#111] text-white" : "text-[#6F6F6B] hover:text-[#111]"
            }`}>
            ...
            <button onClick={() => setView("archive")} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              view === "archive" ? "bg-[#111] text-white" : "text-[#6F6F6B] hover:text-[#111]"
            }`}>
          ...
          {view === "active" && (
            <>
              <Button onClick={() => { if (confirm("모든 활성 AI 액션을 dismissed 처리할까요?")) clearAllMutation.mutate(); }} disabled={clearAllMutation.isPending}
                variant="outline"
                className="flex items-center gap-2 text-sm px-4 py-2 rounded-lg border border-[#DEDEDA] text-[#6F6F6B] hover:text-red-600 hover:border-red-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: "Monitoring", value: monitoring.length, icon: Eye, color: "text-[#6F6F6B]", bg: "bg-[#F1F1EF]" },
          ...
          <div key={label} className="bg-white border border-[#DEDEDA] rounded-xl p-4 flex items-center gap-3">
            ...
            <div><div className="text-2xl font-bold text-[#111]">{value}</div><div className="text-xs text-[#6F6F6B]">{label}</div></div>
          </div>
        ))}
      </div>
      {view === "archive" ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center"><Archive className="w-4 h-4 text-[#6F6F6B]" /></div>
            <div>
              <h2 className="text-base font-semibold text-[#111]">Archived Actions</h2>
              <p className="text-xs text-[#6F6F6B]">Historical actions — older entries and duplicates moved here automatically</p>
            </div>
          </div>
          {isArchiveLoading ? (
        ...
        <div className="space-y-10">
          {pendingApproval.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="flex items-center gap-2"><h2 className="text-base font-semibold text-[#111]">Awaiting Your Approval</h2><span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full font-medium">{pendingApproval.length}</span></div>
                  <p className="text-xs text-[#6F6F6B]">Medium-to-high risk actions that require your sign-off before execution</p>
                </div>
              </div>
          ...
          {autoExecuted.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="flex items-center gap-2"><h2 className="text-base font-semibold text-[#111]">Auto-Executed</h2><span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full font-medium">{autoExecuted.length}</span></div>
                  <p className="text-xs text-[#6F6F6B]">Low-risk optimizations applied automatically — undo any within 24 hours</p>
                </div>
              </div>
          ...
          {monitoring.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-lg bg-[#F1F1EF] flex items-center justify-center"><TrendingUp className="w-4 h-4 text-[#6F6F6B]" /></div>
                <div>
                  <div className="flex items-center gap-2"><h2 className="text-base font-semibold text-[#111]">Monitoring</h2><span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full font-medium">{monitoring.length}</span></div>
                  <p className="text-xs text-[#6F6F6B]">Items being tracked — insufficient data for action or awaiting next sync</p>
                </div>
              </div>
