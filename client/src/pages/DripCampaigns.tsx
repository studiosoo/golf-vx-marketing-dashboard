...
function StatusBadge({ status }: { status: string }) {
  ...
  if (normalized === "scheduled") {
    return (
      <span className="inline-block px-2 py-0.5 bg-[#6F6F6B]/10 text-[#1A56DB] text-[11px] font-medium rounded">
        Scheduled
      </span>
    );
  }
  return (
    <span className="inline-block px-2 py-0.5 bg-[#E9E9E6] text-[#6F6F6B] text-[11px] font-medium rounded">
  ...
  return (
    <div className="bg-white border border-[#DEDEDA] rounded-[10px] p-4 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[#6F6F6B]">{icon}</span>
        <span className="text-xs text-[#6F6F6B]">{label}</span>
      </div>
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#6F6F6B] mt-0.5">Encharge 이메일 브로드캐스트 및 캠페인</p>
        </div>
        <button
      ...
      <div className="bg-white border border-[#DEDEDA] rounded-[10px] shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
        <div className="px-4 py-3 border-b border-[#DEDEDA]">
          ...
          {summary?.lastSyncedAt && (
            <p className="text-xs text-[#A8A8A3] mt-0.5">
        ...
        {/* Loading */}
        {broadcastsLoading && (
          <div className="flex items-center justify-center py-16 gap-2 text-[#6F6F6B]">
        ...
        {!broadcastsLoading && (!broadcasts || broadcasts.length === 0) && (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-center px-4">
            <Mail size={32} className="text-[#A8A8A3]" />
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#DEDEDA]">
                  <th className="text-xs text-[#A8A8A3] font-normal px-4 py-2 text-left">캠페인 이름</th>
                  <th className="text-xs text-[#A8A8A3] font-normal px-4 py-2 text-left">상태</th>
                  <th className="text-xs text-[#A8A8A3] font-normal px-4 py-2 text-right">오픈율</th>
                  <th className="text-xs text-[#A8A8A3] font-normal px-4 py-2 text-right">클릭율</th>
                  <th className="text-xs text-[#A8A8A3] font-normal px-4 py-2 text-right">발송일</th>
                </tr>
              </thead>
              <tbody>
                {broadcasts.map((b) => (
                  <tr
                    key={b.id}
                    className="h-14 border-b border-[#E9E9E6] last:border-0 hover:bg-[#F1F1EF] transition-colors duration-100"
                  >
                    <td className="px-4">
                      ...
                      {b.subject && (
                        <div className="text-xs text-[#6F6F6B] truncate max-w-[240px]">
                          {b.subject}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 text-sm text-[#6F6F6B] text-right whitespace-nowrap">
                      {formatDate(b.sendAt)}
                    </td>
                  </tr>
                ))}
            ...
            {segments.map((seg) => (
              ...
              >
                ...
                <span className="text-sm font-bold text-[#222222]">
                  ...
                  <span className="text-xs text-[#6F6F6B] font-normal ml-1">명</span>
                </span>
              </div>
            ))}
          </div>
