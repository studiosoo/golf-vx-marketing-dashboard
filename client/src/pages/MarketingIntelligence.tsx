    ...
    <div className="bg-white border border-[#DEDEDA] rounded-xl p-4 space-y-3 hover:border-[#F2DD48]/50 transition-colors">
      ...
      {expanded && (
        <div className="pt-2 border-t border-[#F0F0F0] space-y-2">
          {action.reasoning && (
            <div>
              <div className="text-xs font-semibold text-[#6F6F6B] mb-1">Reasoning</div>
              <p className="text-xs text-[#6F6F6B] leading-relaxed">{action.reasoning}</p>
            </div>
          )}
          {action.confidence != null && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#6F6F6B]">Confidence:</span>
    ...
    <div className="p-6 space-y-6 bg-[#F6F6F4] min-h-full">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-[#6F6F6B] mt-1">Autonomous decision engine — analyzes campaigns and takes action</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => clearStaleMutation.mutate()}
            disabled={clearStaleMutation.isPending}
            className="flex items-center gap-1.5 px-3 py-2 text-xs text-[#6F6F6B] border border-[#DEDEDA] bg-white hover:bg-[#F1F1EF] rounded-lg transition-colors disabled:opacity-50"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(syncStatus).map(([key, status]: [string, any]) => (
            <div key={key} className="bg-white border border-[#DEDEDA] rounded-xl p-3">
              ...
              {status?.lastSync && <div className="text-xs text-[#A8A8A3] mt-0.5">{new Date(status.lastSync).toLocaleTimeString()}</div>}
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: "Awaiting Approval", value: approvalCards.length, icon: Clock, color: "text-[#6F6F6B]", bg: "bg-[#6F6F6B]/10" },
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="w-6 h-6 animate-spin text-[#F2DD48]" /><span className="ml-2 text-[#6F6F6B] text-sm">Loading actions…</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full">{autoExecuted.length}</span>
                </div>
                <p className="text-xs text-[#6F6F6B]">Low-risk actions applied automatically</p>
              </div>
            </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-lg bg-[#6F6F6B]/10 flex items-center justify-center"><Clock className="w-4 h-4 text-[#6F6F6B]" /></div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-[#111]">Awaiting Approval</h2>
                  <span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full">{approvalCards.length}</span>
                </div>
                <p className="text-xs text-[#6F6F6B]">Medium/high-risk actions needing review</p>
              </div>
            </div>
            <div className="space-y-3">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-lg bg-[#F1F1EF] flex items-center justify-center"><Eye className="w-4 h-4 text-[#6F6F6B]" /></div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-[#111]">Monitoring</h2>
                  <span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-2 py-0.5 rounded-full">{monitoring.length}</span>
                </div>
                <p className="text-xs text-[#6F6F6B]">Campaigns with insufficient data</p>
              </div>
            </div>
            <div className="space-y-3">
              {monitoring.length === 0 ? (
