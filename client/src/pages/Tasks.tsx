...
const PRIORITY_STYLES: Record<ActionPriority, { label: string; className: string }> = {
  ...
  medium: { label: "Medium",  className: "bg-[#6F6F6B]/10 text-[#6F6F6B] border border-blue-200" },
...
function TypeIcon({ type }: { type: ActionType }) {
  ...
  return (
    <div className="w-9 h-9 rounded-lg bg-[#F1F1EF] flex items-center justify-center flex-shrink-0">
      <Icon className="w-4 h-4 text-[#6F6F6B]" />
    </div>
  );
}
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {action.status === "skipped" && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-[#F1F1EF] text-[#6F6F6B] border border-[#DEDEDA]">
                Skipped
              </span>
            )}
            <span className="text-xs text-[#A8A8A3] capitalize">{action.type.replace("_", " ")}</span>
          </div>

          <p className="text-sm font-semibold text-[#222222] leading-snug">{action.title}</p>
          <p className="text-xs text-[#6F6F6B] mt-0.5 leading-relaxed line-clamp-2">{action.description}</p>

          {(action.expectedImpact || action.effortRequired) && (
            <div className="flex items-center gap-3 mt-1.5 text-xs text-[#A8A8A3]">
              {action.expectedImpact && (
                <span>Impact: <span className="text-[#6F6F6B]">{action.expectedImpact}</span></span>
              )}
              {action.effortRequired && (
                <span>Effort: <span className="text-[#6F6F6B]">{action.effortRequired}</span></span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {!isDone && (
            <>
              ...
              <button
                ...
                disabled={isLoading}
                className="flex items-center gap-1 px-2.5 py-1.5 bg-[#F1F1EF] hover:bg-[#E8E8E8] text-[#6F6F6B] border border-[#DEDEDA] rounded text-xs font-medium transition-colors disabled:opacity-50"
              >
          ...
          <button
            ...
            className="p-1.5 text-[#A8A8A3] hover:text-[#222222] hover:bg-[#F1F1EF] rounded transition-colors"
          >
            <ChevronDown
      ...
      {expanded && (
        <div className="pt-2 border-t border-[#F0F0F0] space-y-1.5">
          ...
          {action.result && (
            <p className="text-xs text-[#6F6F6B] italic">Result: {action.result}</p>
          )}
        </div>
      )}
    </div>
    ...
    <div className="bg-white border border-[#DEDEDA] rounded-[10px] p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm font-bold text-[#222222]">
          {completed} / {total}
          <span className="text-xs font-normal text-[#6F6F6B] ml-1">completed</span>
        </span>
      </div>
      <div className="h-2 bg-[#E9E9E6] rounded-full overflow-hidden">
        <div
          className="h-full bg-[#F2DD48] rounded-full transition-all duration-500"
          style={{ width: `${pct}%` }}
        />
      </div>
      <p className="text-xs text-[#A8A8A3] mt-1.5">{pct}% complete</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="w-14 h-14 rounded-full bg-[#F1F1EF] flex items-center justify-center mb-4">
        <Target className="w-6 h-6 text-[#A8A8A3]" />
      </div>
      <p className="text-base font-semibold text-[#222222] mb-1">No plan for today</p>
      <p className="text-sm text-[#6F6F6B] mb-6 max-w-xs">
    ...
    <div className="p-6 space-y-6 bg-[#F6F6F4] min-h-full">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#6F6F6B] mt-0.5">{todayLabel}</p>
        </div>

        <div className="flex items-center gap-2 flex-wrap">
          {/* Campaign dropdown */}
          {campaignsLoading ? (
            <div className="h-9 w-48 bg-[#E9E9E6] rounded-[10px] animate-pulse" />
          ) : (
            <select
              ...
              <button
                ...
                className={`px-4 text-sm transition-all duration-200 flex items-center gap-1.5 ${
                  activeTab === key
                    ...
                    : "text-[#6F6F6B] font-normal hover:text-[#222222]"
                }`}
              >
                {label}
                {count > 0 && (
                  <span className="text-xs bg-[#F1F1EF] border border-[#DEDEDA] text-[#6F6F6B] px-1.5 py-0.5 rounded-full font-medium">
                    {count}
                  </span>
                )}
              </button>
      ...
      {!selectedCampaignId ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <p className="text-sm text-[#6F6F6B]">Select a campaign to view today's task plan.</p>
        </div>
      ) : planLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="w-6 h-6 animate-spin text-[#F2DD48]" />
          <span className="ml-2 text-[#6F6F6B] text-sm">Loading plan…</span>
        </div>
      ) : !plan || totalCount === 0 ? (
        <EmptyState onGenerate={handleGenerate} isGenerating={generateMutation.isPending} />
      ) : filteredActions.length === 0 ? (
        <div className="text-center py-12 text-[#A8A8A3]">
