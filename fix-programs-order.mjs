import { createConnection } from './node_modules/mysql2/promise/index.cjs';

const url = process.env.DATABASE_URL;
const conn = await mysql.createConnection(url);

try {
  // Fix 1: Update goalType for Trial Session from 'leads' to 'lead_generation'
  await conn.execute('UPDATE campaigns SET goalType = ? WHERE id = 4', ['lead_generation']);
  console.log('✓ Fixed goalType for Trial Session (leads → lead_generation)');
